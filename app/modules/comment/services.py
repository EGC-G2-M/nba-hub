...
class CommentService(BaseService):
    ...
    def delete_comment(self, comment_id: int, acting_user_id: int):
        comment = self.comment_repository.find_by_id(comment_id)
        if not comment:
            raise ValueError("Comment not found.")

        dataset = DataSet.query.get(comment.dataset_id)
        if not dataset:
            raise ValueError("Associated dataset not found.")
        
        is_writer = (comment.user_id == acting_user_id)
        is_dataset_owner = (dataset.user_id == acting_user_id)

        if not is_writer and not is_dataset_owner:
    ...
    def toggle_pin(self, comment_id: int, pin: bool, acting_user_id: int) -> Comment:
        comment = self.comment_repository.find_by_id(comment_id)
        if not comment:
            raise ValueError("Comment not found.")
        ...
        if not is_dataset_owner:
            raise PermissionError("Only the dataset owner can pin comments.")
